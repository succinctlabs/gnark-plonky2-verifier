...
func (c *TestVerifierChallengesCircuit) Define(api frontend.API) error {
	...
	c.fieldAPI = field.NewFieldAPI(api)
	c.qeAPI = field.NewQuadraticExtensionAPI(c.fieldAPI, commonCircuitData.DegreeBits)
	c.hashAPI = poseidon.NewHashAPI(c.fieldAPI)
	poseidonChip := poseidon.NewPoseidonChip(api, c.fieldAPI, c.qeAPI)

...
type PoseidonState = [SPONGE_WIDTH]*FTarget
type PoseidonStateExtension = [SPONGE_WIDTH]*QETarget

type PoseidonChip struct {
	api      frontend.API           `gnark:"-"`
	fieldAPI FieldAPI               `gnark:"-"`
	qeAPI    *QuadraticExtensionAPI `gnark:"-"`
}

func NewPoseidonChip(api frontend.API, fieldAPI FieldAPI, qeAPI *QuadraticExtensionAPI) *PoseidonChip {
...
func (c *PoseidonChip) HashNToMNoPad(input []*FTarget, nbOutputs int) []*FTarget {
...
func (c *PoseidonChip) HashNoPad(input []*FTarget) Hash {
	var hash Hash
...
func (c *PoseidonChip) PartialRounds(state PoseidonState, roundCounter *int) PoseidonState {
	...
	for i := 0; i < N_PARTIAL_ROUNDS; i++ {
		...
		state[0] = c.fieldAPI.Add(state[0], NewFieldConst(FAST_PARTIAL_ROUND_CONSTANTS[i]))
...
func (c *PoseidonChip) ConstantLayer(state PoseidonState, roundCounter *int) PoseidonState {
	for i := 0; i < 12; i++ {
		if i < SPONGE_WIDTH {
			roundConstant := NewFieldConst(ALL_ROUND_CONSTANTS[i+SPONGE_WIDTH*(*roundCounter)])
...
func (c *PoseidonChip) ConstantLayerExtension(state PoseidonStateExtension, roundCounter *int) PoseidonStateExtension {
	for i := 0; i < 12; i++ {
		if i < SPONGE_WIDTH {
			roundConstant := c.qeAPI.FieldToQE(NewFieldConst(ALL_ROUND_CONSTANTS[i+SPONGE_WIDTH*(*roundCounter)]))
...
func (c *PoseidonChip) SBoxMonomial(x *FTarget) *FTarget {
...
func (c *PoseidonChip) SBoxMonomialExtension(x *QETarget) *QETarget {
...
func (c *PoseidonChip) MdsRowShfExtension(r int, v [SPONGE_WIDTH]*QETarget) *QETarget {
	res := c.qeAPI.FieldToQE(NewFieldConst(0))

	for i := 0; i < 12; i++ {
		if i < SPONGE_WIDTH {
			matrixVal := c.qeAPI.FieldToQE(NewFieldConst(MDS_MATRIX_CIRC[i]))
			res1 := c.qeAPI.MulExtension(v[(i+r)%SPONGE_WIDTH], matrixVal)
			res = c.qeAPI.AddExtension(res, res1)
		}
	}

	matrixVal := c.qeAPI.FieldToQE(NewFieldConst(MDS_MATRIX_DIAG[r]))
...
func (c *PoseidonChip) MdsLayer(state_ PoseidonState) PoseidonState {
	...
	for i := 0; i < SPONGE_WIDTH; i++ {
		result[i] = NewFieldConst(0)
	}

	var state [SPONGE_WIDTH]frontend.Variable
	for i := 0; i < SPONGE_WIDTH; i++ {
		state[i] = c.api.FromBinary(c.fieldAPI.ToBits(state_[i])...)
	}
...
func (c *PoseidonChip) PartialFirstConstantLayer(state PoseidonState) PoseidonState {
	for i := 0; i < 12; i++ {
		if i < SPONGE_WIDTH {
			state[i] = c.fieldAPI.Add(state[i], NewFieldConst(FAST_PARTIAL_FIRST_ROUND_CONSTANT[i]))
		}
	}
	return state
}
...
func (c *PoseidonChip) PartialFirstConstantLayerExtension(state PoseidonStateExtension) PoseidonStateExtension {
	for i := 0; i < 12; i++ {
		if i < SPONGE_WIDTH {
			state[i] = c.qeAPI.AddExtension(state[i], c.qeAPI.FieldToQE(NewFieldConst(FAST_PARTIAL_FIRST_ROUND_CONSTANT[i])))
		}
	}
	return state
}
...
func (c *PoseidonChip) MdsPartialLayerInit(state PoseidonState) PoseidonState {
	...
	for r := 1; r < 12; r++ {
		if r < SPONGE_WIDTH {
			for d := 1; d < 12; d++ {
				if d < SPONGE_WIDTH {
					t := NewFieldConst(FAST_PARTIAL_ROUND_INITIAL_MATRIX[r-1][d-1])
...
func (c *PoseidonChip) MdsPartialLayerInitExtension(state PoseidonStateExtension) PoseidonStateExtension {
	var result PoseidonStateExtension
	for i := 0; i < 12; i++ {
		result[i] = c.qeAPI.FieldToQE(NewFieldConst(0))
	}
	...
	for r := 1; r < 12; r++ {
		if r < SPONGE_WIDTH {
			for d := 1; d < 12; d++ {
				if d < SPONGE_WIDTH {
					t := c.qeAPI.FieldToQE(NewFieldConst(FAST_PARTIAL_ROUND_INITIAL_MATRIX[r-1][d-1]))
...
func (c *PoseidonChip) MdsPartialLayerFast(state PoseidonState, r int) PoseidonState {
	...
	for i := 1; i < 12; i++ {
		if i < SPONGE_WIDTH {
			...
			si := c.api.FromBinary(c.fieldAPI.ToBits(state[i])...)
			dSum = c.api.Add(dSum, c.api.Mul(si, t))
		}
	}

	s0 := c.api.FromBinary(c.fieldAPI.ToBits(state[0])...)
	...
	for i := 1; i < 12; i++ {
		if i < SPONGE_WIDTH {
			t := NewFieldConst(FAST_PARTIAL_ROUND_VS[r][i-1])
...
func (c *PoseidonChip) MdsPartialLayerFastExtension(state PoseidonStateExtension, r int) PoseidonStateExtension {
	s0 := state[0]
	mds0to0 := c.qeAPI.FieldToQE(NewFieldConst(MDS_MATRIX_CIRC[0] + MDS_MATRIX_DIAG[0]))
	d := c.qeAPI.MulExtension(s0, mds0to0)
	for i := 1; i < 12; i++ {
		if i < SPONGE_WIDTH {
			t := c.qeAPI.FieldToQE(NewFieldConst(FAST_PARTIAL_ROUND_W_HATS[r][i-1]))
	...
	for i := 1; i < 12; i++ {
		if i < SPONGE_WIDTH {
			t := c.qeAPI.FieldToQE(NewFieldConst(FAST_PARTIAL_ROUND_VS[r][i-1]))
